...
- **구조적 검증(강제)** — `publish_post`가 결정론적으로 막는다: 섹션·영역 정합성, 본문 최소 분량(`minChars`), `##` 소제목, 이중 프론트매터, 그리고 `follows`/`related`와 **본문 내부 링크**가 **존재하지 않는 글을 가리키면 발행 거부**(끊긴 간선·링크 할루시네이션 차단). 본문 링크는 형태까지 본다 — 글 주소는 `/blog/<slug>/`라 날짜 프리픽스를 빠뜨리면 404가 된다.
