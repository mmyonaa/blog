...
증상이 특징적이다. 서버 프로세스는 죽지 않고 살아 있는데, 클라이언트 쪽에서 "Unexpected token", "invalid JSON", "connection closed" 같은 메시지가 뜬다. 그리고 그 시점이 **서버가 무언가를 stdout에 찍는 순간과 겹친다**. 초기화 직후 바로 깨진다면 시작 로그를, 특정 도구를 부를 때만 깨진다면 그 도구 안의 출력을 의심하면 된다.

내가 짠 코드만 문제가 아니다. **의존 라이브러리**가 stdout에 찍는 경우도 있다. 어떤 패키지는 경고나 배너를 `console.log`로 흘리는데, 이것도 똑같이 스트림을 오염시킨다. 그래서 stdio 서버에서는 "내 코드가 stdout에 안 찍나"뿐 아니라 "쓰는 라이브러리가 안 찍나"까지 확인해야 한다.
