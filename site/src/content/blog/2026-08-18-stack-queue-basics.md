...
section: "jeongcheogi"
tags: ["정보처리기사", "자료구조", "스택", "큐", "원형큐"]
related: ["2026-08-02-bfs-dfs", "2026-08-15-cpu-scheduling"]
description: "스택은 마지막에 넣은 것이 먼저 나오는 LIFO, 큐는 먼저 넣은 것이 먼저 나오는 FIFO 구조다. push·pop과 enqueue·dequeue 연산 추적, 스택으로 만들 수 없는 출력 순열, 원형 큐의 포화·공백 조건까지 정보처리기사 필기에서 반복 출제되는 포인트를 정리한다."
---

스택과 큐는 데이터를 넣고 빼는 순서에 규칙을 둔 선형 자료구조다. 정보처리기사 필기에서는 연산을 몇 번 수행한 뒤 남는 값이나 출력 순서를 추적하는 문제, 스택으로 만들 수 있는 출력 순열, 원형 큐의 포화·공백 조건이 반복해서 나온다.
