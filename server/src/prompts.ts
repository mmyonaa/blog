...
const STYLE_RULES: string[] = [
  ...
  `   - 수식·의사코드는 코드블록에: 수식·의사코드·프로토콜 메시지처럼 줄바꿈과 등폭이 의미를 가지는 것은 반드시 \`\`\`text 코드블록에 넣는다(예: \`C_i = E_k(P_i XOR C_(i-1))\`). 본문 문단에 그대로 두면 여러 줄이 한 줄로 합쳐져 붙어 나온다. 한 줄짜리 짧은 식·식별자·옵션명은 인라인 코드(백틱)로 감싼다.`,
];

const SELF_REVIEW_ITEMS: string[] = [
  `   1) 사실 접지: 확신 없이 단정한 수치·버전·날짜·인용·통계가 있나? 있으면 불확실성을 드러내거나 뺀다.`,
  `   2) 코드/명령 실행성: 예제가 그대로 실행되나? 존재하지 않는 API·옵션·미검증 플래그는 없나? 코드블록 밖 본문에 여러 줄 수식·의사코드를 흘려 둔 곳은 없나?`,
